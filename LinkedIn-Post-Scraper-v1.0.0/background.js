...
function scraperMain(startDate, endDate) {
  ...
  function extractURL(card) {
    ...
    var profileEl = card.querySelector('a[href*="/in/"], a[href*="/company/"]');
    if (profileEl) {
  ...
  function extractName(card) {
    ...
    if (/^Feed post/i.test(cardTxt)) {
      ...
      if (lines.length > 1 && lines[1].length > 1 && lines[1].length < 100 &&
          !/^\d/.test(lines[1]) &&
    ...
    for (var a = 0; a < actorContainers.length; a++) {
      var link = actorContainers[a].querySelector('a[href*="/in/"], a[href*="/company/"]');
      if (link) {
